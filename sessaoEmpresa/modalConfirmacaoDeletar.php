<?php
    ...
    if (isset($_SESSION['displayModalConfirDeletar']) && $_SESSION['displayModalConfirDeletar']) {
    $cd_servico = $_SESSION['cd_servico'];

    echo '<div id="myModalConfirDeletar" class="modal" style="display: block;">';
    echo '<div class="modal-content">';
    echo '<h2>Deseja deletar esse serviço?</h2>';
    echo '<button onclick="confirmDeletar()">Sim</button>';
    echo '<button onclick="cancelDeletar()">Não</button>';
    echo '</div>';
    echo '</div>';

    unset($_SESSION['displayModalConfirDeletar']);
    
    echo '<script>
            function confirmDeletar() {
                var totalPedidosAtivos = <?php echo $totalPedidosAtivos; ?>;

                if (totalPedidosAtivos > 0) {
                    
                    window.location.href = "perfilEmpresa.php?cd_servico=<?php echo $cd_servico; ?>&deletar=1";
                } else {
                    // Não há pedidos ativos com o serviço, exclua-o
                    window.location.href = "modalDeletar.php?cd_servico=<?php echo $cd_servico; ?>";
                }
            }

            function cancelDeletar() {
